...
void *testproc(int arg1, void *arg2) {
...
#if CS402TESTS > 7
    dbg(DBG_PRINT, "proc kill test");
    for ( i=0 ; i < 10; i++ )
	start_proc(NULL, "kill all test", cancelme_test, 0);
    stop_until_queued(10, &wake_me_len);
    /*
     * Well, can't really call proc_kill_all() because it will kill
     * this process as well; so, do the following instead.
     */
    ...
    dbg(DBG_PRINT, "All tests completed\n\n");
    return NULL;
}
